...
const service = {
  ...
  inventoryReports: () => FIND_MANY_RESULT,
  inventoryReport: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("InventoryReport", () => {
  ...
  test("POST /inventoryReports", async () => {
    await request(app.getHttpServer())
      .post("/inventoryReports")
      .send(CREATE_INPUT)
  ...
  test("GET /inventoryReports", async () => {
    await request(app.getHttpServer())
  ...
  test("GET /inventoryReports/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/inventoryReports"}/${nonExistingId}`)
  ...
  test("GET /inventoryReports/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/inventoryReports"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /inventoryReports existing resource", async () => {
